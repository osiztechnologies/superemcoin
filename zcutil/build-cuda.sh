...
./autogen.sh
CC="$CC" CXX="$CXX" ./configure --enable-gpu --with-cudalib --prefix="${PREFIX}" --host="$HOST" --build="$BUILD" "$RUST_ARG" "$HARDENING_ARG" "$LCOV_ARG" "$TEST_ARG" "$MINING_ARG" "$PROTON_ARG" "$LIBS_ARG" --enable-werror CXXFLAGS='-g'
"$MAKE" "$@" V=1
