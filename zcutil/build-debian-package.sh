...
PACKAGE_VERSION=$($SRC_PATH/src/supremecoind --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/supremecoind $DEB_BIN
cp $SRC_PATH/src/supremecoin-cli $DEB_BIN
...
# Copy manpages
cp $SRC_DOC/man/supremecoind.1 $DEB_MAN
cp $SRC_DOC/man/supremecoin-cli.1 $DEB_MAN
cp $SRC_DOC/man/zcash-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/supremecoind.bash-completion $DEB_CMP/supremecoind
cp $SRC_PATH/contrib/supremecoin-cli.bash-completion $DEB_CMP/supremecoin-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/supremecoind.1
gzip --best -n $DEB_MAN/supremecoin-cli.1
gzip --best -n $DEB_MAN/zcash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/supremecoind $DEB_BIN/supremecoin-cli
