...
#else
    const char* pszModule = "Supremecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Supremecoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "supremecoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override supremecoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "supremecoind.pid"));
...
{
    return "\n" +
           FormatParagraph(strprintf(_("In order to ensure you are adequately protecting your privacy when using Supremecoin, please see <%s>."),
...
std::string LicenseInfo()
{
    return "\n" +
           ...
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i Supremecoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
