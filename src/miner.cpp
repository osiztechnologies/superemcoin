...
#ifdef ENABLE_WALLET
void static BitcoinMiner(GPUConfig conf, CWallet *pwallet)
#else
void static BitcoinMiner(GPUConfig conf)
#endif
{
    ...
    if( conf.useGPU )
    {
        ...
        {
            LogPrintf("Error in eq_cuda_context: %s\n",e.what());
            std::lock_guard<std::mutex> lock{g_cs};
            g_cancelSolver = false;
            return;
        }
        LogPrintf("Using Equihash solver GPU\n");
    ...
    try {
        while (true) {
            ...
            if (nHeight == -1) {
                LogPrintf("Error in Supremecoin Miner: chainActive.Height() returned -1\n");
                return;
            }
            ...
            if( conf.useGPU )
                ...
                LogPrint("pow", "Using Equihash solver CPU with n = %u, k = %u\n", n, k);
            ...
            while (true) {
                ...
                // (x_1, x_2, ...) = A(I, V, n, k)
                if( conf.useGPU )
                    LogPrint("pow", "Running Equihash solver GPU with nNonce = %s\n", pblock->nNonce.ToString());
                else
                    LogPrint("pow", "Running Equihash solver CPU with nNonce = %s\n", pblock->nNonce.ToString());
                ...
                if (conf.useGPU) {
                    ...
                    for (size_t i = 0; i < 32; ++i)
                        header[CBlockHeader::HEADER_SIZE-32 + i] = pblock->nNonce.begin()[i];
                    ...
                    {
                        if( g_solver )
                            found = g_solver->solve((unsigned char *)pblock, header, CBlockHeader::HEADER_SIZE);
                        ehSolverRuns.increment();
                        if (found) 
                            break;
...
#endif
{
    ...
    conf.selGPU = GetArg("-device", 0);

#ifdef ENABLE_GPU    
    conf.allGPU = GetBoolArg("-allgpu", 0);
    conf.sel_platform = GetArg("-platform", 0);
    conf.useCUDA   = GetBoolArg("-CUDA", 0);
    ...
    {
        if(conf.useGPU) {
            nThreads = 1;   // limit max GPU thread per device 1;  
        }
        else
        ...
        if(conf.allGPU) {
            ...
            for(unsigned platform = 0; platform < numPlatforms; ++platform) 
            {
                ...
                for(unsigned device = 0; device < noDevices; ++device) {
                    ...
                    LogPrintf("GenerateBitcoins GPU (platform=%d device=%d) maxThread =%d.\n", conf.currentPlatform, conf.currentDevice, maxThreads );

                    if ( !bNvidiaDev )
                        continue;

                    for (int i = 0; i < maxThreads; i++){
                        LogPrintf("GenerateBitcoins GPU (platform=%d device=%d) starting ...\n", conf.currentPlatform, conf.currentDevice);
#ifdef ENABLE_WALLET
                        minerThreads->create_thread(boost::bind(&BitcoinMiner, conf, pwallet));
#else
                        minerThreads->create_thread(boost::bind(&BitcoinMiner, conf));
#endif
                    }
                }
            }
            ...
            if (devicesFound <= 0) {
                LogPrintf("GenerateBitcoins ERROR, No GPU devices found!\n");
            }
        } 
        else
        {
            LogPrintf("GenerateBitcoins GPU @(platform=%d device=%d) only!\n", conf.currentPlatform, conf.currentDevice );
            ...
            if ( bNvidiaDev  && devices.size() > conf.currentDevice) 
            {
            ...
            {
                LogPrintf("GenerateBitcoins ERROR, No OpenCL devices found!\n");
            }
        }
#endif
    }
