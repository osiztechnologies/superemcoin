...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or supremecoin.conf setting gen)\n"
...
static bool cb_validate(std::vector<unsigned char> sols, unsigned char *pblockdata, int thrid)
{
    ...
    do 
    {
        ...
        LogPrintf("genesis nonce: %s\n", pblock->nNonce.ToString());
        LogPrintf("genesis solution: %s\n", HexStr(sols.begin(),sols.end()));
...
UniValue generate(const UniValue& params, bool fHelp)
{
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "supremecoind compiled without wallet and -mineraddress not set");
#endif
    }
    ...
    {        
        ...
        LogPrint("pow", "Using Equihash solver GPU\n");
    }    
#endif
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Supremecoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Supremecoin is downloading blocks...");
