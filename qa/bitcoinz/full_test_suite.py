...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/supremecoin/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/supremecoin/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/supremecoind')
    ret &= test_rpath_runpath('src/supremecoin-cli')
    ret &= test_rpath_runpath('src/zcash-gtest')
    ret &= test_rpath_runpath('src/supremecoin-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')
    ret &= test_rpath_runpath('src/zcash/GenerateParams')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/supremecoind')
    ret &= test_fortify_source('src/supremecoin-cli')
    ret &= test_fortify_source('src/zcash-gtest')
    ret &= test_fortify_source('src/supremecoin-tx')
